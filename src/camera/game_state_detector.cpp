...
// RE7's managers do not answer to the fully-qualified names RE2 and RE3 use, so
// the gate probes for them by shape instead.
//
// That leaves the title, main-menu and loading screens, which render over a
// live 3D backdrop and so satisfy every probe. The one unambiguous signal there
// is that a menu GUI element drew, which gui_compensation.cpp reports.
